...
<table class='species_stats_table'>
   <thead>
      <tr>
         <td>Match type <a href='#stats_help'>[?]</a></td>         
...
<?php 
  ...
  foreach ($overall_stats as $stat) {    
    ...
    if( $stat['match_type'] != 'not matched' ) {
      ...
      $additional_shortcode_key = "";
      if( stripos($match_type_shortcode, "[") ) {
        $additional_shortcode_key = substr($match_type_shortcode,stripos($match_type_shortcode, "[")+1);        
        $additional_shortcode = str_replace("]","",$additional_shortcode_key);
        $additional_shortcode = str_replace("/","_",$additional_shortcode);
        $match_type_shortcode = substr($match_type_shortcode,0,stripos($match_type_shortcode, "[")-1);
      }
      $shown[$stat['match_type']] = $match_type_shortcode;
      if( $additional_shortcode_key != '' ) $shown[$additional_shortcode_key] = $additional_shortcode;
      echo "<td>" . ucfirst($stat['match_type']) . "</td>";      
    } else {
...
<a name='stats_help'></a>
<div id='stats_help_box'>
   <b>Help:</b><br/>
<?php
$shown = array_unique($shown);
foreach( $shown as $key => $shortcode ) {
  echo do_shortcode("[infopopup tag=" . $shortcode . "]") . "<br/>";
}
?>
</div>
